...
class  Transaction: ults_Transaction {
    ...
    func doChallenge(_ applicationContext: ults_Context?, _ challengeParameters: ults_ChallengeParameters, _ challengeStatusReceiver: ults_ChallengeStatusReceiver, _ timeOut: Int) throws {
        ...
        if let userlogin = headerDic["x5c"] as? NSString {
            var asd = userlogin.components(separatedBy: ",")
            asd[0] = asd[0].replacingOccurrences(of: "[", with: "")
            asd[0] = asd[0].replacingOccurrences(of: "]", with: "")
            asd[0] = asd[0].replacingOccurrences(of: " ", with: "")
            asd[1] = asd[1].replacingOccurrences(of: "[", with: "")
            asd[1] = asd[1].replacingOccurrences(of: "]", with: "")
            asd[1] = asd[1].replacingOccurrences(of: " ", with: "")
            certis = asd
        }
        ...
        if verified {
            ...
            let acsURL = acsEphemPubKeydic["acsURL"] as! String
            if let acsEphem = acsEphemPubKeydic["acsEphemPubKey"] as? [String : String] {
                ...
                if gcm {
                    ...
                    AbstractNetRequest().post(stringData:makefinal , url: acsURL, transation: self, challengeParameters: challengeParameters, challengeStatusReceiver: challengeStatusReceiver, ui: nil)
                } else {
    ...
    //func sendCreq(challengeType : String ,htmldata : String) {
    //
    //if Creqjson.acsUiType == "01" {
    //
    //let cReq = CReq.init(sdkCounterStoA: Creqjson.acsCounterAtoS, acsTransID: Creqjson.acsTransID , sdkTransID: Creqjson.sdkTransID, threeDSServerTransID: Creqjson.threeDSServerTransID, challengCancel: "", challengDataEntry: "abc", challengHTMLDataEntry: "", messageExtension: "", oobContinue: true, challengWindowSize: "", messageType: "CReq")
    //
    //print(acsURL)
    //
    //let makefinal = getJWE128GCM(acstransId: challengeParameters.getAcsTransactionID(), sharesecret: sharesecret, get3DSServerTransactionID: challengeParameters.get3DSServerTransactionID(), sdkCounter: 0)
    //AbstractNetRequest().post(stringData:makefinal , url: acsURL, transation: self)
    //}
    //}
    ...
    //func  getkey() -> [String:AnyObject] {
    //var publicKey: SecKey?
    //var privateKey: SecKey?
    //var publicKeyAttr: [NSObject: NSObject] = [
    ...
    //if statusCode == noErr && publicKeyAttr != nil && privateKeyAttr != nil {
...
class  ProgressView: ults_ProgressView {
    ...
    func show() {
        let spinnerActivity = MBProgressHUD.showAdded(to: ((UIApplication.shared.delegate?.window)!)!, animated: true)
...
class AuthenticationRequestParameters: ults_AuthenticationRequestParameters {
    ...
    init() {
        ...
        self.sdkAppID = "99e65b97-fcfd-4012-a87c-db3411fb7d9d"
